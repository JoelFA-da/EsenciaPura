...
class EmailService {
    ...
    async sendBookingConfirmation(data) {
        ...
        `
            ...
            logger_1.logger.info(`Email de confirmación enviado a ${data.customerEmail}`);
        }
        catch (error) {
            logger_1.logger.error(`Error al enviar email de confirmación: ${error}`);
            throw error;
        }
    }
    ...
    async verifyConnection() {
        try {
            ...
            logger_1.logger.info('Conexión SMTP verificada exitosamente');
            return true;
        }
        catch (error) {
            logger_1.logger.error(`Error al verificar conexión SMTP: ${error}`);
            return false;
        }
    }
}
