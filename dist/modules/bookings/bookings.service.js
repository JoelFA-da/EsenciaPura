...
class BookingsService {
    ...
    async create(data) {
        ...
        const endTime = (0, date_fns_1.format)(endDate, 'HH:mm');
        // Validar disponibilidad SOLO si no es admin forzando reserva
        if (!data.skipAvailabilityCheck) {
            const isAvailable = await this.checkAvailability(data.date, data.startTime, endTime);
            if (!isAvailable) {
        ...
        return await prisma.booking.create({
            data: {
                ...
                date: (0, date_fns_1.parseISO)(data.date),
    ...
    async list(filters) {
        return await prisma.booking.findMany({
            where: {
                ...
                ...(filters?.date && { date: (0, date_fns_1.parseISO)(filters.date) })
            },
            include: {
                service: true
            },
    ...
    async getAvailableSlots(date, serviceId) {
        ...
        while (currentHour < endHour ||
            ...
            const slotEnd = (0, date_fns_1.format)(slotEndDate, 'HH:mm');
    ...
    async getCalendarView(date) {
        ...
        while (currentHour < endHour ||
            ...
            const booking = bookings.find((b) => b.startTime === timeSlot);
            if (booking) {
                slots.push({
                    time: timeSlot,
                    isAvailable: false,
